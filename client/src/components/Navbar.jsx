...
function MenuList({ user }) {
  ...
  const handleProfileClick = () => {
    // Redirect to the appropriate profile page based on account type
    const profilePath = user?.accountType ? "/user-profile" : "/company-profile";
  ...
  return (
    <Menu as='div' className='inline-block text-left'>
      <div className='relative inline-block text-left'>
        <Menu.Button className='flex gap-2 items-center w-full rounded-md bg-white md:px-4 py-2 text-sm font-medium text-slate-700 hover:bg-opacity-20'>
          <div className="flex flex-col items-start">
            <p className='text-sm font-semibold'>{user?.firstName ?? user?.name}</p>
            <p className='text-sm text-blue-600'>{user?.jobTitle ?? user?.email}</p>
          </div>
          <img
            src={user?.profileUrl}
            alt="user profile"
        ...
        <nav className="container mx-auto flex items-center justify-between p-5">
          <div>
            <Link to="/" className="text-blue-600 font-bold text-xl">
              Job<span className="text-[#1677cccb]">Dash</span>
            </Link>
          </div>
          <ul className="hidden lg:flex gap-10 test-base">
            <li>
            ...
            {!user?.token ? (
              <Link to="/user-auth">
                <CustomButton
                  title="Sign In"
                  containerStyles={`
                    text-blue-600 py-1.5 px-5 focus:outline-none
                    hover:bg-blue-700 hover:text-white rounded-full text-base border
                    border-blue-600
                  `}
                />
              </Link>
